...
final class FirestoreManager {
  ...
  /// 서브컬렉션의 특정 문서에서 일부 필드를 업데이트합니다.
  /// - Parameters:
  ///   - parentType: 부모 컬렉션(.users 등)
  ///   - parentId: 부모 문서 ID
  ///   - subCollection: 서브컬렉션(.userNotification 등)
  ///   - documentId: 업데이트할 문서 ID
  ///   - asDictionary: 업데이트할 필드 데이터
  func updateFieldsInSubcollection(
    ...
    var updateData = asDictionary
    updateData[WriteStrategy.update.rawValue] = FieldValue.serverTimestamp()
  ...
  /// 특정 유저의 안 읽은 알림 개수를 가져옵니다.
  /// - Parameters:
  ///   - userId: 유저의 document ID
  ///   - within: 알림을 포함하는 부모 컬렉션 (예: .users)
  ///   - subCollection: 서브 컬렉션 이름 (예: .userNotification)
  ///   - limitToRecentMonth: 최근 1개월만 포함할지 여부 (기본 true)
  /// - Returns: 안 읽은 알림 개수 (Int)
  func fetchUnreadNotificationCount(
    for userId: String,
    within parentCollection: CollectionType = .users,
    subCollection: CollectionType = .userNotification,
    limitToRecentMonth: Bool = true
  ) async throws -> Int {
    
    let parentRef = db
      .collection(parentCollection.rawValue)
      .document(userId)
    ...
    if limitToRecentMonth {
      ...
      query = query.whereField("created_at", isGreaterThanOrEqualTo: Timestamp(date: oneMonthAgo))
    }
