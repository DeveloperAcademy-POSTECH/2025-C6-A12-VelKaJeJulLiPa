...
struct TracksInlineView: View {
  ...
  @Bindable var viewModel: TracksListViewModel
  ...
  var body: some View {
    ...
    .alert(
      "트랙을 삭제하시겠어요?",
      isPresented: Binding(
        get: { viewModel.alertState.isPresentingDeleteAlert },
        set: { viewModel.alertState.isPresentingDeleteAlert = $0 }
      )
    ) {
  ...
  private var contentView: some View {
    ...
    } else if let errorText = viewModel.dataState.errorText {
      Text(errorText)
        .font(.footnote)
        .foregroundStyle(.red)
    ...
    } else if viewModel.dataState.tracks.isEmpty {
      Text("곡이 없습니다. 추가해보세요")
        .font(.body1Medium)
  ...
  // MARK: - 트랙 카드 뷰
  private var tracksCardView: some View {
    VStack(spacing: 0) {
      ForEach(Array(viewModel.dataState.tracks.enumerated()), id: \.element.tracksId) { _, track in
        
        let rowState = viewModel.perRowState(for: track.tracksId)
        let canEdit  = true   // 나중에 권한 체크 필요 시 viewModel.canEdit(track:) 등으로 분리
        
        TrackRow(
          track: track,
          rowState: rowState,
          deleteAction: {
            // viewModel.requestDelete(track: track)
          },
          editAction: {
            // viewModel.startEditing(track: track)
          },
          rowTapAction: {
...
//            )))
          ...
          editText: Binding(
            get: {
              (viewModel.editingState.editingId == track.tracksId)
              ? viewModel.editingState.editText
              : track.trackName
            },
            set: { newValue in
              if viewModel.editingState.editingId == track.tracksId {
                viewModel.editingState.editText = newValue
              }
            }
          ),
          canEdit: canEdit
...
//            let canEdit = true /*(track.creatorId == currentUserId || teamspaceOwner == currentUserId)*/
//            
//            TrackRow(
//              track: track,
