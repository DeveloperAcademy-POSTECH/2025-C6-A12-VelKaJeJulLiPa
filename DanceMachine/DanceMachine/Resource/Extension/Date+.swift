...
extension Date {
  ...
  func listTimeLabel(now: Date = .now) -> String {
    ...
    if diff < 7 * 24 * 60 * 60 {
      ...
      formatter.unitsStyle = .full           // "13분 전", "1시간 전", "어제"
    ...
    let cachedDateFormatter = Date.cachedDateFormatter
    cachedDateFormatter.locale = .current
    cachedDateFormatter.dateFormat = sameYear ? "M월 d일" : "yyyy. M. d."
    return cachedDateFormatter.string(from: self)
  }
